...
- **무조건 버그가 발생하는 지점이 없는지 항상 꼼꼼히 점검합니다.** 코드를 바꾸거나 조사할 때, 확실히 깨지는 지점(null/undefined 참조, 경계값, 타입 불일치, 비동기 순서, 예외 미처리, 기존 동작 회귀 등)을 능동적으로 찾아 스스로 검증하고, 발견 시 임의로 넘기지 말고 명시적으로 보고합니다. "아마 될 것" 대신 근거로 확인합니다.
