...
- **무조건 문제가 생기는 지점이 없는지 항상 꼼꼼히 점검합니다.** 스펙·결정을 낼 때 확실히 깨지거나 어긋나는 지점(계약 불일치, 데이터 정합성 깨짐, 논리적 구멍, 엣지 케이스 누락)을 능동적으로 짚고, 발견 시 임의로 넘기지 말고 명시적으로 보고합니다. 데이터 근거가 있으면 "아마 그럴 것" 대신 조회해 확인합니다.
