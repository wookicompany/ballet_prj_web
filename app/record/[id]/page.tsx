...
export default function RecordDetailPage() {
  ...
  return (
    <MobileContainer>
      {deleting || deletingRecurrence ? <LoadingOverlay /> : null}
            ...
            <div className="flex items-center justify-between gap-3 rounded-2xl border border-dashed border-[#17171c]/20 bg-[#17171c]/5 px-4 py-3">
              ...
              <div className="flex shrink-0 gap-2">
                ...
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  className="h-11 px-4 text-xs text-red-500"
                  onClick={() => {
                    sendHapticToApp();
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>
                {isPlanned
                  ? "이 반복의 예정을 모두 삭제할까요?"
                  : "이 반복의 남은 예정을 모두 삭제할까요?"}
              </AlertDialogTitle>
              <AlertDialogDescription>
                완료하지 않은 예정은 지난 것도 함께 삭제돼요. 감정을 기록해 완료한 날은 그대로 남아요.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter className="flex flex-row gap-2">
              <AlertDialogCancel className="flex-1">취소</AlertDialogCancel>
