...
export default function EditCenterOrderPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const res = await fetch(`/api/saved-center-orders/${id}`, {
      ...
      const { item } = (await res.json()) as { item: SavedCenterOrder };
      setName(item.name);
      setOrderTags(
        item.order_text
  ...
  const handleSubmit = async () => {
    ...
    const response = await fetch(`/api/saved-center-orders/${id}`, {
      method: "PATCH",
      headers: {
    ...
    <MobileContainer>
      <main className="flex min-h-screen flex-col px-4 pb-8">
        <header className="sticky top-0 z-20 bg-white h-12 mb-6 flex items-center justify-between">
          ...
          <h1 className="text-base font-semibold">센터 순서 수정</h1>
          <div className="w-10" />
        </header>
