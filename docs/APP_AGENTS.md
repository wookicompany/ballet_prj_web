...
- 색상 팔레트는 다음을 기본으로 사용한다.
  - 브랜드 색상: `#E8517C` (최소한으로 사용, `--brand` 토큰만 사용)
  - 알림 신호색: `#FF154A` (`--alert` 토큰. 미읽음/알림 대비 전용, 브랜드색으로 통일하지 않는다)
  - 베이스 다크: `#17171c`
  - 베이스 라이트: `#FFFFFF`
- **브랜드 강조색은 `--brand`(#E8517C) 토큰만 사용한다. 미읽음/알림 신호색은 브랜드와 분리된 `--alert` 토큰(현행 #FF154A)을 사용하며, 브랜드색으로 통일하지 않는다(알림 대비 우선).**
...
| `lib/profileCache.ts` | `/profile` | 기록·리뷰 생성·수정·삭제, 브랜드 좋아요 토글. payload에 미리보기 데이터(`recordsPreview` 3개, `reviewsPreview` 3개, `likedBrandsPreview` 3개) 포함. 서브페이지(`/profile/records`, `/profile/reviews`, `/profile/brands`)는 캐시 없이 매번 fresh fetch |
