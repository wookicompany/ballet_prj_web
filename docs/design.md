...
| 완료 — 감정 없음 | 채운 원 | `h-10 w-10 rounded-full bg-[#17171c]/10` ("감정 기록 없이 완료"한 기록. 무드 이미지가 없으므로 채운(solid) 원으로 완료를 표기 — 예정의 점선(dashed) 원과 대비) |
...
- **완료 표기 구분**: 감정 있는 완료는 무드 얼굴, 감정 없는 완료는 채운 원, 예정은 점선 원 — solid(완료) vs dashed(예정)로 상태를 구분한다. 감정 없는 완료 기록도 캘린더 셀에서 반드시 보여야 한다(무표기로 두면 "완료가 안 됐나?" 오인).
...
- 종료일: 기존 `Calendar` 컴포넌트 재사용, **3개월 상한 이후 날짜는 기존 "비활성" 스타일 그대로 선택 차단**(`text-muted-foreground opacity-50`) — 캡을 UI에서 원천 차단하므로 별도 경고 문구 불요(§9.5).
...
- 스킵·캡 없이 단순 성공: `sonner` toast 1줄(카피는 아래 표).
- 스킵/캡이 하나라도 발생: BottomSheet로 결과 요약(§9.8).
...
┌───────────────────────────────────────────────────┐
│  [PenLine]     수정하기                              (기본)
│  [CheckCircle] 무드 없이 완료 처리  ← planned일 때만   (기본, 빨강 아님)
│  [Trash2]      삭제하기                              (빨강)
│  [Trash2]      남은 예정 전체 삭제 / 다음 예정 전체 삭제  ← recurrence_id 있을 때만 (빨강)
└───────────────────────────────────────────────────┘
```

- "무드 없이 완료 처리"는 완료 자체가 파괴적 액션이 아니므로(monotonic 경고만 필요) 텍스트를 빨강으로 두지 않는다. 선택 시 기존 `AlertDialog` 패턴(Title + Description + `AlertDialogCancel`/`AlertDialogAction`, `app/record/[id]/page.tsx`의 삭제 확인 다이얼로그와 동일 구조) 재사용, description은 카피 표 참조.
- "남은 예정 전체 삭제"/"다음 예정 전체 삭제"는 대량 삭제이므로 기존 "삭제하기"와 동일하게 `text-red-500`.

**5) 예정 미디어 섹션**

- 예정 상태에서는 미디어 업로드 섹션을 **완전히 렌더링하지 않는다**(회색조 비활성이 아니라 숨김).
- 그 자리에 Caption 토큰(`text-[13px] text-[#17171c]/50`)으로 안내 한 줄만 노출: "완료하면 사진을 추가할 수 있어요."
...
| 무드 없이 완료 확인(AlertDialog) | "완료 후에는 예정으로 되돌릴 수 없어요." |
| 0회 생성(스킵, 등록 버튼은 선계산으로 원천 비활성) | "선택한 요일과 종료일로는 예정을 만들 수 없어요." (toast, 서버 0건 응답 시) |
| 생성 성공(스킵 없음) | "예정 N개를 만들었어요." (toast) |
| 생성 성공(스킵/캡 있음) | 결과 요약 BottomSheet — "예정 N개를 만들었어요" / "겹치는 날짜 M일은 건너뛰었어요" / "3개월까지만 등록할 수 있어요" |
| 지난(놓친) 예정 caption | **"지난 예정이에요"** — "지난 일정"이 아닌 "지난 예정"으로 통일(기능 용어 "예정"과 일관). |
| 지난 예정 액션 | [완료로 표시] [삭제] |
| 예정 미디어 섹션 대체 caption | "완료하면 사진을 추가할 수 있어요." |
| 남은 전체 삭제(예정 카드에서) | "남은 예정 전체 삭제" |
| 남은 전체 삭제(완료 카드에서) | "다음 예정 전체 삭제" |
| 삭제 확인 AlertDialog(항상 표시) | "직접 시간 바꾼 예정도 함께 삭제돼요. 완료된 기록은 삭제되지 않아요." |
