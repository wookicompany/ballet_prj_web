...
### TimePickerSheet (`components/sheets/TimePickerSheet.tsx`)

- `BottomSheet` 위에 만든 공용 시간 선택 컴포넌트(2026-09 개선② 시간 피커 재설계 도입). 기록 작성·수정·반복 예정 등록 화면의 시작/종료 시간 피커가 전부 이 컴포넌트를 쓴다.
- [오전/오후] · [시 1~12] · [분 00~59] 3열. 모두 `Button`(선택 `variant="default"` / 비선택 `"ghost"`, 기본 size `h-9`, `w-full justify-start`)으로 구성해 오전/오후도 다른 두 컬럼과 동일하게 실제 클릭 가능하다.
- 오전/오후 컬럼은 항목이 2개뿐이라, 시·분 목록 컬럼과 같은 박스 높이(`h-48`) 안에서 `flex flex-col justify-center`로 수직 중앙 정렬한다 — 상단에 쏠려 아래에 빈 여백이 남는 것을 방지.
- **시·분 목록 행 높이(`h-9` = 36px)는 레이아웃 가이드라인의 최소 터치 타깃 44px 규칙의 의도적 예외다.** 12개(시)·60개(분) 항목을 스크롤 가능한 좁은 컬럼 안에 배치해야 하므로, 44px을 쓰면 목록이 과도하게 길어진다. 컬럼 폭이 좁고 세로 스크롤이 있어 오탭 위험이 낮다는 전제로 허용한다.
- 오전/오후만 눌러도 이미 고른 시·분은 초기화되지 않는다(meridiem만 갱신).
- 입출력은 항상 24시간 "HH:MM"(2자리 zero-pad) — 저장 포맷과 동일하다. 내부적으로 12시간 표기로 보여주되, 적용 시 24시간으로 변환해 돌려준다(경계값: 오전 12시→"00", 오후 12시→"12").

### 시간 표기 규칙

- **입력·상세** (TimePickerSheet 트리거 버튼, 기록 상세 시간 표기 등): 12시간, **시(hour)는 zero-pad 없이**(분은 2자리 유지) — 예: "오후 6시 30분", "오후 6시 05분". 자정(0시)은 "오전 12시", 정오(12시)는 "오후 12시"로 표기하고 "자정"·"정오"라는 단어 자체는 쓰지 않는다.
- **목록·타임라인·어드민** (캘린더 셀, day 타임라인, 어드민 records 등): 24시간 — 예: "18:30".
- **저장(DB)**: 항상 24시간 "HH:MM"(2자리 zero-pad).
- 공용 변환 함수는 `lib/kstDateTime.ts`에 있다 — `formatKoreanTimeLabel(time)`(24h "HH:MM" → "오후 6시 30분" 형식), `getMeridiemLabel(hour24)` / `getHour12(hour24)`(24h hour → 오전/오후, 1~12시). 입력·상세 화면은 전부 이 함수로 통일하고, 화면마다 다른 포맷 함수를 새로 만들지 않는다.
...
> 근거: `docs/PRD/ticket-book.md` §7. 신규 컴포넌트는 만들지 않고 기존 컴포넌트(`PageHeader`·`Button`·`Badge`·`BottomSheet`·`AnimatedImage`·`Spinner`)만 재사용한다. 진입점은 공연 탭 헤더 우측의 티켓 아이콘 버튼.

### 2단 스티키 헤더 (`app/ticket-book/page.tsx`)

- 1단은 `PageHeader`(뒤로가기 + "티켓북"), 2단은 그 아래 붙는 연월·등록 줄이다. 캘린더 탭 헤더 구조를 그대로 따른다.
- 2단 컨테이너: `sticky top-12 z-20 flex h-12 items-center justify-between bg-background px-4` — `top-12`는 1단 헤더 높이(48px)와 같아야 스크롤 시 두 줄이 붙어 고정된다.
- 왼쪽은 연월 선택 버튼(탭하면 `BottomSheet`로 연/월 선택), 오른쪽은 티켓 등록(`+`) 버튼.
- **연월 텍스트는 1단 헤더 타이틀보다 크면 안 된다.** 타이틀이 화면의 이름이고 연월은 그 안의 현재 위치라, 크기가 역전되면 위계가 뒤집힌다.

### 캘린더 셀

| 항목 | 값 |
|---|---|
| 셀 높이 | `h-[116px]` — 포스터가 들어가므로 기록 캘린더보다 높다 |
| 셀 구성 | `flex flex-col items-center justify-start gap-1.5 pt-1.5` |
| 날짜 숫자 | `h-7 w-7 rounded-full text-sm` |
| 오늘 | `bg-[#17171c] text-white` (검정 원 + 흰 글씨) — 기록 캘린더와 완전히 동일 |
| 선택 | 셀에 `rounded-md bg-[#17171c]/5`, 날짜 숫자는 `font-bold` |
| 포스터 | `h-[60px] w-[42px] rounded-md object-cover` |
| 포스터 없음 | 같은 크기의 `bg-[#17171c]/5` 플레이스홀더 |
| 같은 날 2건 이상 | 포스터 우상단에 `Badge`(`absolute -top-1 right-0.5 min-w-6 rounded-full bg-primary text-xs text-white`)로 건수 표기 |

- **건수 배지의 앵커는 셀 전체가 아니라 포스터 크기(`relative h-[60px] w-[42px]`)에 맞춘다.** 셀 폭을 기준으로 두면 좁은 포스터 오른쪽 허공에 배지가 뜬다.
- 썸네일 우선순위: KOPIS 포스터 → 첨부 이미지 첫 장 → 플레이스홀더. 판정은 반드시 truthy 체크로 한다 — `kopis_performances.poster`는 NULL이 아니라 **빈 문자열**인 행이 있어 `!== null`로 짜면 `<img src="">`가 렌더된다.

### 월 전환 스와이프

- 캘린더 영역을 좌우로 밀어 월을 바꾼다. 임계값 `SWIPE_THRESHOLD_PX = 40`.
- 전환 직후 `SWIPE_TRANSITION_LOCK_MS = 240` 동안 잠근다. 없으면 빠르게 여러 번 밀 때 월이 두세 칸씩 건너뛴다.

### 별점 (`app/ticket-book/new/detail/page.tsx`, `app/ticket/[id]/edit/page.tsx`)

공연 상세의 리뷰 작성 별점과 **완전히 동일한 마크업**을 쓴다. 화면마다 다르게 만들지 않는다.

- 별 5개, 각 별이 2점 — 저장값은 0~10의 짝수. 반쪽 별 표현은 없다(오버레이 폭이 `100%` 또는 `0%`).
- 컨테이너 `relative h-7 w-7`, 아이콘 `h-6 w-6 text-brand`.
- **비활성 별도 색이 있다.** 바닥 별은 `text-brand` + `fill="none"`(윤곽선만), 활성분은 그 위에 `fill="currentColor"` 오버레이를 폭으로 덮는다. 회색으로 빼지 않는다.
- 별점은 선택 항목이라 **이미 선택된 최상위 별을 다시 탭하면 해제**된다(0점). "선택 안 함" 같은 별도 버튼은 두지 않는다.
- 별 옆에 숫자(점수)를 병기하지 않는다.
- 오버레이 구조상 클릭 타깃이 네이티브 `<button>`이므로 `sendHapticToApp()`을 직접 호출해야 한다(`Button` 컴포넌트는 내부에서 자동 호출).

### 티켓 등록·수정 화면 구성

섹션 순서는 **공연 정보 → 미디어 업로드 → (직접 입력이면 공연명·장소) → 관람 날짜 → 좌석 → 별점 → 내용·공개 토글**.

- 공연 정보는 선택한 공연을 먼저 확인시키는 카드다. 직접 입력이라 확인할 공연 정보가 없으면 미디어 업로드를 맨 위에 둔다.
- 미디어 업로드는 기록 생성(`app/record/new/page.tsx`)과 동일한 구조를 재사용한다.
- **리뷰 영역에 섹션 제목과 구분선을 두지 않는다.** "여기서부터 별개의 무거운 작업"이라는 신호가 되어 리뷰 작성의 심리적 문턱을 만든다. 공개 여부의 경계는 기본값 OFF인 공개 토글과 그 아래 설명이 대신한다.
- **별점은 리뷰가 아니라 티켓의 값**이다(리뷰를 안 써도 캘린더와 상세에 뜬다). 그래서 직접 입력 공연에서도 매길 수 있다. 리뷰를 만들 수 없어 숨기는 것은 내용 입력과 공개 토글뿐이다.
- 별점과 아래 요소 사이 간격은 `space-y-6` 기준으로 넉넉히 둔다 — 별이 서로 붙어 보이면 탭 대상이 불분명해진다.
- 메모 필드는 두지 않는다. 메모가 있으면 그 아래 리뷰를 쓰지 않게 된다.

### 날짜 선택 (`components/sheets/DatePickerSheet.tsx`)

- `BottomSheet` 위의 연/월/일 3열 선택. 기록 화면의 인라인 날짜 시트와 같은 구조를 공용 컴포넌트로 뺀 것이다.
- **월을 바꿀 때 일(day)은 그 달의 말일로 줄인다**(`Math.min(draft.day, daysInMonth)`). 보정하지 않으면 1월 31일 → 2월에서 `"YYYY-02-31"`이 만들어져 저장이 거부된다.
- 시트 내용은 열렸을 때만 마운트한다(닫힌 상태에서 스크롤 위치를 맞추려 effect에서 setState하면 린트 에러).

### "오늘" 판정

- `formatSeoulDateKey()`로 계산하고, `visibilitychange`에서 화면이 다시 보일 때 재계산한다. 앱을 열어둔 채 자정을 넘기면 어제 칸에 "오늘" 배지가 남기 때문이다.
- 캘린더 탭과 달리 **보고 있던 달과 선택 날짜는 되돌리지 않는다.** 티켓북은 지난 달을 되돌아보는 화면이라 복귀할 때마다 이번 달로 튕기면 안 된다.
...
|---|---|
| 화면 이름 | "티켓북" |
| 대상 지칭 | "관람한 공연" ("본 공연" 아님) |
| 관람 날짜 미입력 | "관람 날짜를 확인해 주세요." |
| 리뷰만 쓰고 별점 없음 | "리뷰를 쓰려면 별점도 선택해 주세요." |
| 공연 정보 로드 실패 | "공연 정보를 불러오지 못했어요. 잠시 후 다시 시도해 주세요." |
